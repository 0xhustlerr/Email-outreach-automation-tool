...
//
// WebGL fluid simulation, adapted from PavelDoGreat / RunicFreak.
// React lifecycle wraps it so it cleans up on unmount / HMR re-runs.
// Color palette is constrained to the cyan–teal–blue range to match the app.
//
"use client";
...
export default function FluidBackground() {
  ...
  useEffect(() => {
    ...
    // --- Event handlers (retained for cleanup) ---------------------
    let firstMove = false;
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
      const color = firstMove ? pointer.color : generateColor();
      if (!firstMove) {
        firstMove = true;
      }
    ...
    // Constrained cyan → teal → blue palette, with a small purple accent chance.
    function generateColor() {
      // Hue bands: cyan (.48), teal (.45), azure (.55), purple accent (.75)
      const bands = [0.48, 0.5, 0.52, 0.55, 0.58, 0.75];
      const h = bands[Math.floor(Math.random() * bands.length)];
    ...
    // Wire up events & kick off the loop.
    resizeCanvas();
  ...
  return (
    <canvas
      ref={canvasRef}
      className="pointer-events-none fixed inset-0 z-[2] h-screen w-screen"
